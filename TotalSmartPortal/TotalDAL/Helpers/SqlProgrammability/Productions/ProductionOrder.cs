...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class ProductionOrder
    {
        ...
        private void GetProductionOrderPendingPlannedOrders()
        {
            ...
            queryString = queryString + "                       INNER JOIN Customers ON PlannedOrders.PlannedOrderID IN (SELECT PlannedOrderID FROM FirmOrderDetails WHERE LocationID = @LocationID AND Approved = 1 AND InActive = 0 AND InActivePartial = 0 AND ROUND(Quantity - QuantitySemifinished, " + (int)GlobalEnums.rndQuantity + ") > 0) AND PlannedOrders.PlannedOrderID NOT IN (SELECT PlannedOrderID FROM ProductionOrderDetails WHERE InActive = 0 AND InActivePartial = 0) AND PlannedOrders.CustomerID = Customers.CustomerID " + "\r\n";
        ...
        private void GetProductionOrderPendingCustomers()
        {
            ...
            queryString = queryString + "       FROM           (SELECT DISTINCT CustomerID FROM PlannedOrders WHERE PlannedOrderID IN (SELECT PlannedOrderID FROM FirmOrderDetails WHERE LocationID = @LocationID AND Approved = 1 AND InActive = 0 AND InActivePartial = 0 AND ROUND(Quantity - QuantitySemifinished, " + (int)GlobalEnums.rndQuantity + ") > 0) AND PlannedOrders.PlannedOrderID NOT IN (SELECT PlannedOrderID FROM ProductionOrderDetails WHERE InActive = 0 AND InActivePartial = 0)) CustomerPENDING " + "\r\n";

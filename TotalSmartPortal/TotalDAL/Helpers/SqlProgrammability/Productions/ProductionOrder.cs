...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class ProductionOrder
    {
        ...
        private void GetProductionOrderIndexes()
        {
            ...
            queryString = queryString + "       SELECT      ProductionOrders.ProductionOrderID, CAST(ProductionOrders.EntryDate AS DATE) AS EntryDate, ProductionOrders.Reference, ProductionOrders.Code, ProductionOrders.Specs, Locations.Code AS LocationCode, ISNULL(PlannedOrders.Reference, ProductionOrders.PlannedOrderReferences) AS PlannedOrderReference, ISNULL(PlannedOrders.Code, ProductionOrders.PlannedOrderReferences) AS PlannedOrderCode, PlannedOrders.VoucherDate AS PlannedOrderVoucherDate, PlannedOrders.DeliveryDate AS PlannedOrderDeliveryDate, ISNULL(Customers.Name, N'LSX tổng hợp') AS CustomerName, ISNULL(VoidTypes.Name, CASE ProductionOrders.InActivePartial WHEN 1 THEN N'Hủy một phần đh' ELSE N'' END) AS VoidTypeName, ProductionOrders.Description, ProductionOrders.Approved, ProductionOrders.InActive, ProductionOrders.InActivePartial " + "\r\n";

...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class SemifinishedProduct
    {
        ...
        private void GetSemifinishedProductViewDetails()
        {
            ...
            queryString = queryString + "       SELECT      ISNULL(SemifinishedProductDetails.SemifinishedProductDetailID, 0) AS SemifinishedProductDetailID, FirmOrderDetails.FirmOrderDetailID, FirmOrderDetails.PlannedOrderDetailID, Commodities.CommodityID, Commodities.Code AS CommodityCode, Commodities.Name AS CommodityName, Commodities.CommodityTypeID, " + "\r\n";
            ...
            queryString = queryString + "                   LEFT OUTER JOIN SemifinishedProductDetails ON SemifinishedProductDetails.SemifinishedProductID = @SemifinishedProductID AND FirmOrderDetails.FirmOrderDetailID = SemifinishedProductDetails.FirmOrderDetailID " + "\r\n";
        ...
        private void GetSemifinishedProductPendingMaterialIssueDetails()
        {
            ...
            queryString = queryString + "       SELECT          MaterialIssueDetails.MaterialIssueDetailID, MaterialIssueDetails.MaterialIssueID, MaterialIssueDetails.FirmOrderID, FirmOrders.EntryDate AS FirmOrderEntryDate, FirmOrders.Reference AS FirmOrderReference, FirmOrders.Code AS FirmOrderCode, FirmOrders.Specification, " + "\r\n";
            queryString = queryString + "                       MaterialIssueDetails.CustomerID, Customers.Code AS CustomerCode, Customers.Name AS CustomerName, Workshifts.EntryDate AS WorkshiftEntryDate, Workshifts.Code AS WorkshiftCode, MaterialIssueDetails.ProductionLineID, ProductionLines.Code AS ProductionLineCode, ROUND(MaterialIssueDetails.Quantity - MaterialIssueDetails.QuantitySemifinished, " + (int)GlobalEnums.rndQuantity + ") AS QuantityRemains " + "\r\n";

            queryString = queryString + "       FROM            MaterialIssueDetails  " + "\r\n";
            queryString = queryString + "                       INNER JOIN FirmOrders ON MaterialIssueDetails.LocationID = @LocationID AND ROUND(MaterialIssueDetails.Quantity - MaterialIssueDetails.QuantitySemifinished, " + (int)GlobalEnums.rndQuantity + ") > 0 AND MaterialIssueDetails.FirmOrderID = FirmOrders.FirmOrderID " + "\r\n";

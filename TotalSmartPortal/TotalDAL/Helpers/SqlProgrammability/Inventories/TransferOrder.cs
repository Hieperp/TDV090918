...
namespace TotalDAL.Helpers.SqlProgrammability.Inventories
{
    public class TransferOrder
    {
        ...
        private void GetTransferOrderIndexes()
        {
            ...
            queryString = queryString + "       SELECT      TransferOrders.TransferOrderID, CAST(TransferOrders.EntryDate AS DATE) AS EntryDate, TransferOrders.Reference, Locations.Code AS LocationCode, Customers.Name AS CustomerName, TransferOrders.TransferOrderTypeID, TransferOrderTypes.Name AS TransferOrderTypeName, TransferOrders.AdjustmentJobs, ISNULL(VoidTypes.Name, CASE TransferOrders.InActivePartial WHEN 1 THEN N'Hủy một phần đh' ELSE N'' END) AS VoidTypeName, TransferOrders.Description, TransferOrders.TotalQuantity, TransferOrders.Approved " + "\r\n";
        ...
        private void GetTransferOrderViewDetails()
        {
            ...
            queryString = queryString + "       DECLARE     @TransferOrderDetails TABLE (TransferOrderDetailID int NOT NULL, TransferOrderID int NOT NULL, CommodityID int NOT NULL, Quantity decimal(18, 2) NOT NULL, Remarks nvarchar(100) NULL, VoidTypeID int) " + "\r\n";
            queryString = queryString + "       INSERT INTO @TransferOrderDetails (TransferOrderDetailID, TransferOrderID, CommodityID, Quantity, Remarks, VoidTypeID) SELECT TransferOrderDetailID, TransferOrderID, CommodityID, Quantity, Remarks, VoidTypeID FROM TransferOrderDetails WHERE TransferOrderID = @TransferOrderID " + "\r\n";

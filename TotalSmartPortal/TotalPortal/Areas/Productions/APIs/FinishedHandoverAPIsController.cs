...
namespace TotalPortal.Areas.Productions.APIs
{
    ...
    public class FinishedHandoverAPIsController : Controller
    {
        ...
        public JsonResult GetPendingDetails([DataSourceRequest] DataSourceRequest dataSourceRequest, int? finishedHandoverID, int? plannedOrderID, int? customerID, string finishedProductDetailIDs, bool? isReadonly)
        {
            var result = this.finishedHandoverAPIRepository.GetPendingDetails(finishedHandoverID, plannedOrderID, customerID, finishedProductDetailIDs, false);
            return Json(result.ToDataSourceResult(dataSourceRequest), JsonRequestBehavior.AllowGet);
        }
    }
}